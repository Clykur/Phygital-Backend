-- Legacy Supabase restores pointed Phygital child rows at archive._legacy_pre_enterprise_users.
-- The API auth user lives in public.users; repoint FKs (idempotent per constraint name).

-- Drop legacy FKs first so orphan cleanup can run without violating old references.
...
-- Rows tied to archived user ids cannot be owned by API-authenticated users until repointed.
